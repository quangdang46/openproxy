...
function buildUserContent(message: Message): string | Array<{ type: string; text?: string; image_url?: { url: string } }> {
...
function readAssistantText(chunk: any): string {
...
function normalizeStaticModel(model: any, connection: Connection): Model | null {
...
function normalizeLiveModel(model: any, connection: Connection): Model | null {
...
function parseProviderModelsPayload(data: any): any[] {
...
function pickPreferredModel(group: ProviderGroup | null): Model | null {
...
export default function BasicChatPageClient() {
  ...
  const ensureSessionForModel = (model: Model): Session | null => {
