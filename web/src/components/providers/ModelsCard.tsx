...
interface ModelsCardProps {
  providerId: string;
  kindFilter?: string; // e.g. "tts", "embedding" — filters models shown
  providerAliasOverride?: string; // override alias (e.g. for custom-embedding nodes using prefix)
}
...
export default function ModelsCard({ providerId, kindFilter, providerAliasOverride }: ModelsCardProps) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (connRes.ok) setConnections((connData.connections || []).filter((c: { provider: string }) => c.provider === providerId));
          ...
          {displayModels.map((model) => {
            ...
            return (
              <ModelRow
                ...
                copied={copied}
                onCopy={copy}
                onSetAlias={(alias: string) => handleSetAlias(model.id, alias)}
