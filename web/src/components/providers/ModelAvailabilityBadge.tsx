...
export default function ModelAvailabilityBadge(): React.ReactNode {
  ...
  const handleClearCooldown = async (provider: string, model: string): Promise<void> => {
                ...
                {Object.entries(byProvider).map(([provider, provModels]) => (
                  <div key={provider}>
                    ...
                    <div className="flex flex-col gap-1">
                      {provModels.map((m) => {
                        const status = STATUS_CONFIG[m.status as keyof typeof STATUS_CONFIG] || STATUS_CONFIG.unknown;
