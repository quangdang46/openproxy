...
export default function ProviderDetailPage() {
  ...
  useEffect(() => {
    setMounted(true);
    setParams({ id: window.location.pathname.split("/").pop() || "" });
  }, []);
  ...
  const [loadingRemoteModels, setLoadingRemoteModels] = useState<boolean>(false);
  const [selectedModelIds, setSelectedModelIds] = useState<string[]>([]);
  const [savingSelectedModels, setSavingSelectedModels] = useState<boolean>(false);
  ...
  const handleUpdateNode = async (formData: Record<string, any>) => {
    try {
  ...
  const handleToggleModelSelected = (modelId: string) => {
  ...
  const fetchRemoteModels = useCallback(async () => {
    ...
    try {
      ...
      const deduped = Array.from(
        new Map(parsed.map((item: { id: string; name?: string }) => [item.id, item])).values()
      );
  ...
  const handleSaveApiKey = async (formData: { name: string; apiKey: string; priority: number; testStatus: string }) => {
    try {
  ...
  const handleSwapPriority = async (conn1: Connection, conn2: Connection) => {
...
function ModelRow({ model, fullModel, alias, selected, onToggleSelect, copied, onCopy }: ModelRowProps) {
  return (
...
function PassthroughModelsSection({ providerAlias, modelAliases, copied, onCopy, onSetAlias, onDeleteAlias }: PassthroughModelsSectionProps) {
...
function PassthroughModelRow({ modelId, fullModel, copied, onCopy, onDeleteAlias }: PassthroughModelRowProps) {
  return (
...
function CompatibleModelsSection({ providerStorageAlias, providerDisplayAlias, modelAliases, copied, onCopy, onSetAlias, onDeleteAlias, connections, isAnthropic }: CompatibleModelsSectionProps) {
  const [newModel, setNewModel] = useState<string>("");
  const [adding, setAdding] = useState<boolean>(false);
...
function ConnectionRow({ connection, isOAuth, isFirst, isLast, onMoveUp, onMoveDown, onToggleActive, onEdit, onDelete }: ConnectionRowProps) {
...
interface AddApiKeyModalProps {
  ...
  isAnthropic?: boolean;
  onSave: (data: { name: string; apiKey: string; priority: number; testStatus: string }) => Promise<void>;
  onClose: () => void;
}

function AddApiKeyModal({ isOpen, provider, providerName, isCompatible, isAnthropic, onSave, onClose }: AddApiKeyModalProps) {
  const [formData, setFormData] = useState<{ name: string; apiKey: string; priority: number }>({
    name: "",
    apiKey: "",
    priority: 1,
  });
...
function EditConnectionModal({ isOpen, connection, onSave, onClose }: EditConnectionModalProps) {
  const [formData, setFormData] = useState<{ name: string; priority: number; apiKey: string }>({
    name: "",
    priority: 1,
    apiKey: "",
  });
  ...
  const handleSubmit = async () => {
    setSaving(true);
    try {
      const updates: Record<string, any> = { name: formData.name, priority: formData.priority };
      const isOAuth = connection?.authType === "oauth";
      const isCompatible = connection?.provider && (isOpenAICompatibleProvider(connection.provider) || isAnthropicCompatibleProvider(connection.provider));
...
function EditCompatibleNodeModal({ isOpen, node, onSave, onClose, isAnthropic }: EditCompatibleNodeModalProps) {
  const [formData, setFormData] = useState<{ name: string; prefix: string; apiType: string; baseUrl: string }>({
    name: "",
    prefix: "",
    apiType: "chat",
