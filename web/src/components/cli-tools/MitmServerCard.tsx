 ...
 */
export default function MitmServerCard({ apiKeys, cloudEnabled, onStatusChange }: MitmServerCardProps): React.ReactNode {
  ...
  const fetchStatus = useCallback(async (): Promise<void> => {
    try {
      ...
      if (res.ok) {
        const data: MitmStatus = await res.json();
        setStatus(data);
  ...
  const doAction = async (action: string, password: string, forceKillPort443 = false): Promise<void> => {
    setLoading(true);
    setActionError(null);
    try {
      let res: Response;
      ...
      <Card padding="sm" className="border-primary/20 bg-primary/5">
        <div className="flex flex-col gap-3">
          ...
          <div className="px-2 py-2 rounded-lg bg-surface/50 border border-border/50 flex flex-col gap-2">
            ...
            <p className="text-[11px] text-text-muted leading-relaxed">
              <span className="font-medium text-text-main">How it works:</span> Antigravity/Copilot IDE request → DNS redirect to localhost:443 → MITM proxy intercepts → OpenProxy → response to Antigravity/Copilot
            </p>
          </div>
          ...
          <div className="flex flex-col gap-2">
            <div className="grid gap-1 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
              <span className="text-xs font-semibold text-text-main sm:text-right sm:text-sm">OpenProxy Base URL</span>
              <span className="material-symbols-outlined hidden text-text-muted text-[14px] sm:inline">arrow_forward</span>
              <input
                type="text"
                value={mitmRouterBaseUrl}
                onChange={(e: ChangeEvent<HTMLInputElement>) => setMitmRouterBaseUrl(e.target.value)}
            ...
            {!isRunning && (
              <div className="grid gap-1 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
                ...
                <input
                  ...
                  onChange={(e: ChangeEvent<HTMLInputElement>) => setSelectedApiKey(e.target.value)}
                  placeholder={cloudEnabled ? "Enter or pick API key" : "sk_openproxy (default)"}
          ...
          {serverIsWindows && !isAdmin && (
            <div className="flex items-center gap-2 px-2 py-1.5 rounded text-xs bg-red-500/10 text-red-600 border border-red-500/20">
              ...
              <span>Administrator required — restart OpenProxy as Administrator to use MITM</span>
            </div>
          )}
        </div>
      </Card>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="mx-4 flex w-full max-w-sm flex-col gap-4 rounded-xl border border-border bg-surface p-5 shadow-xl sm:p-6">
            ...
            <Input
              ...
              onKeyDown={(e: KeyboardEvent<HTMLInputElement>) => { if (e.key === "Enter" && !loading) handleConfirmPassword(); }}
            />
            {modalError && (
