...
export default function CopilotToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus }: CopilotToolCardProps): React.ReactNode {
  ...
  const removeModel = (id: string): void => setModelList((prev) => prev.filter((m) => m !== id));
  ...
  return (
    <Card padding="sm" className="overflow-hidden">
      <div className="flex items-center justify-between hover:cursor-pointer" onClick={onToggle}>
        <div className="flex items-center gap-3">
          <div className="size-8 flex items-center justify-center shrink-0">
            <img src="/providers/copilot.png" alt={tool.name} width={32} height={32} className="size-8 object-contain rounded-lg" sizes="32px" onError={(e: React.SyntheticEvent<HTMLImageElement>) => { (e.target as HTMLImageElement).style.display = "none"; }} />
          </div>
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  {apiKeys.length > 0 || selectedApiKey ? (
                    <select value={selectedApiKey} onChange={(e: ChangeEvent<HTMLSelectElement>) => setSelectedApiKey(e.target.value)} className="px-3 py-2 bg-bg-secondary rounded-lg text-sm border border-border focus:outline-none focus:ring-1 focus:ring-primary/50">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-[1fr_auto_auto] sm:items-center">
                    <input
                      ...
                      onKeyDown={(e: KeyboardEvent<HTMLInputElement>) => e.key === "Enter" && addModel()}
      ...
      <ModelSelectModal
        ...
        onSelect={(model: { value: string }) => { setModelInput(model.value); setModalOpen(false); }}
