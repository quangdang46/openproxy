...
export default function DroidToolCard({
  ...
  cloudEnabled,
  initialStatus,
}: DroidToolCardProps): React.ReactNode {
  ...
  const checkDroidStatus = async (): Promise<void> => {
    ...
    try {
      ...
      setDroidStatus({ installed: false, error: (error as Error).message });
    } finally {
      ...
      <div className="flex items-start justify-between gap-3 hover:cursor-pointer sm:items-center" onClick={onToggle}>
        <div className="flex min-w-0 items-center gap-3">
          <div className="size-8 flex items-center justify-center shrink-0">
            <img src="/providers/droid.png" alt={tool.name} width={32} height={32} className="size-8 object-contain rounded-lg" sizes="32px" onError={(e: React.SyntheticEvent<HTMLImageElement>) => { (e.target as HTMLImageElement).style.display = "none"; }} />
          </div>
                  ...
                  <div className="flex-1 flex flex-col gap-1">
                    ...
                    <div className="flex items-center gap-1.5">
                      <input
                        ...
                        onKeyDown={(e: KeyboardEvent<HTMLInputElement>) => { if (e.key === "Enter") { e.preventDefault(); addModel(); } }}
