...
export default function MitmToolCard({
  ...
  cloudEnabled,
  onDnsChange,
}: MitmToolCardProps): React.ReactNode {
  ...
  const loadSavedMappings = async (): Promise<void> => {
    try {
  ...
  const saveMappings = useCallback(async (mappings: Record<string, string>): Promise<void> => {
    try {
  ...
  const handleMappingBlur = (alias: string, value: string): void => {
    saveMappings({ ...modelMappings, [alias]: value });
  };

  const handleModelMappingChange = (alias: string, value: string): void => {
  ...
  const handleModelSelect = (model: { value: string; isPlaceholder?: boolean }): void => {
  ...
  const doDnsAction = async (action: string, password: string): Promise<void> => {
    setLoading(true);
    setWarning(null);
    try {
        ...
        {isExpanded && (
          <div className="mt-4 pt-4 border-t border-border flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-0.5 text-[11px] text-text-muted px-1">
              <p>Toggle DNS to redirect {tool.name} traffic through OpenProxy via MITM.</p>
              {!dnsActive && (
                  ...
                  <div key={model.alias} className="grid grid-cols-1 gap-1.5 sm:grid-cols-[9rem_auto_1fr_auto] sm:items-center sm:gap-2">
                    ...
                    <div className="relative w-full min-w-0">
                      <input
                        ...
                        onChange={(e: ChangeEvent<HTMLInputElement>) => handleModelMappingChange(model.alias, e.target.value)}
                        onBlur={(e: ChangeEvent<HTMLInputElement>) => handleMappingBlur(model.alias, e.target.value)}
