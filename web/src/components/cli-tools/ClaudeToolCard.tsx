...
import { stripModelContextMarker } from "@/shared/utils/modelMarkers";
...
export default function ClaudeToolCard({
  ...
  // Sync auto-compact selector with the saved env value.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (!env) return;
    setOneMContext(tool.defaultModels.some((model) => (env[model.envKey as string] as string)?.endsWith("[1m]")));
