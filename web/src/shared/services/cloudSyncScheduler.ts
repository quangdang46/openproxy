...
export class CloudSyncScheduler {
   ...
   */
  async syncWithRetry(maxRetries: number = 1): Promise<unknown | null> {
  ...
  async sync(): Promise<unknown> {
    ...
    if (!response.ok) {
      ...
      throw new Error((errorData as { error?: string }).error || "Sync failed");
    }
...
export async function getCloudSyncScheduler(machineId: string | null = null, intervalMinutes: number = 15): Promise<CloudSyncScheduler> {
