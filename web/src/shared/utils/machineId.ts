 ...
 * 
 * @param salt - Optional salt to use (defaults to environment variable)
 * @returns Machine ID (16-character base32)
 */
export async function getConsistentMachineId(salt: string | null = null): Promise<string> {
  ...
  try {
    ...
    return cryptoModule.randomUUID ? cryptoModule.randomUUID() : 
