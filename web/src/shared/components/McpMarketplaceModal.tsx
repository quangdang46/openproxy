...
export interface McpMarketplaceAddPayload {
  name: string;
  title?: string;
  description?: string;
  url: string;
...
interface McpMarketplaceModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd?: (plugin: McpMarketplaceAddPayload) => void;
  addedNames?: string[];
}
...
export default function McpMarketplaceModal({
  ...
  const fetchTools = async (server: McpMarketplaceServer) => {
    if (toolsCache[server.url]) return;
    setToolsLoading((p) => ({ ...p, [server.url]: true }));
    try {
      // Prefer POST probe (live tools/list). Fall back to registry toolNames on failure.
      ...
      const tools: Array<{ name: string; description?: string }> = Array.isArray(d.tools)
        ? d.tools
        : [];
      ...
      if (fallback.length > 0) {
        setToolSelection((p) => ({
          ...p,
          [server.url]: Object.fromEntries(fallback.map((t) => [t, true])),
        }));
      }
    } finally {
  ...
  const expandServer = (server: McpMarketplaceServer) => {
  ...
  const confirmAdd = (server: McpMarketplaceServer) => {
                  ...
                  <div className="flex items-start gap-2 px-2 py-2 hover:bg-black/5 dark:hover:bg-white/5">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <span className="font-medium text-xs">{s.title || s.name}</span>
                        {s.oauth ? (
