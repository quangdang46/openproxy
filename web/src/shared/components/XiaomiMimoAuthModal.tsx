...
interface XiaomiMimoAuthModalProps {
  isOpen: boolean;
...
interface DetectResult {
  ...
  baseUrl?: string;
  source?: string;
  mimoPassToken?: string | null;
  mimoUserId?: string | null;
  mimoCUserId?: string | null;
  error?: string;
}
 ...
 *
 * Ported from 9router `src/shared/components/XiaomiMimoAuthModal.js`.
 * Backend: `GET /api/oauth/xiaomi-mimo/auto-import` + `POST /api/oauth/xiaomi-mimo/api-key`
 * (Rust `src/server/api/oauth.rs`). The browser-OAuth fallback calls
 * `/api/oauth/xiaomi-mimo/authorize|poll-status|exchange`, which the Rust
 * backend does not implement — those buttons surface the backend error
 * instead of a dead link.
 */
export default function XiaomiMimoAuthModal({ isOpen, onSuccess, onClose }: XiaomiMimoAuthModalProps): React.ReactNode {
  ...
  // Poll OAuth result
  const handlePollOAuth = async (): Promise<void> => {
