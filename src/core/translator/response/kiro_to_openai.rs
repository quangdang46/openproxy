...
pub fn kiro_to_openai_response(chunk: &Value, state: &mut HashMap<String, Value>) -> Option<Value> {
    ...
    let response_id = state.get("responseId").and_then(|v| v.as_str()).unwrap_or("unknown").to_string();
    let created = state.get("created").and_then(|v| v.as_i64()).unwrap_or(0);
    let chunk_idx = state.get("chunkIndex").and_then(|v| v.as_u64()).unwrap_or(0);
    let model = state.get("model").and_then(|v| v.as_str()).unwrap_or("kiro").to_string();

    let event_type = chunk.get("_eventType")
    ...
    if event_type == "assistantResponseEvent" || chunk.get("assistantResponseEvent").is_some() {
        ...
        state.insert("chunkIndex".to_string(), Value::Number((chunk_idx + 1).into()));
    ...
    if event_type == "reasoningContentEvent" || chunk.get("reasoningContentEvent").is_some() {
        ...
        let content = reasoning.get("text")
    ...
    if event_type == "toolUseEvent" || chunk.get("toolUseEvent").is_some() {
        ...
        let tool_call_id = tool_use.get("toolUseId")
            .and_then(|v| v.as_str())
    ...
    if event_type == "usageEvent" || chunk.get("usageEvent").is_some() {
        ...
        if let Some(usage_obj) = usage.as_object() {
            let input_tokens = usage_obj.get("inputTokens").and_then(|v| v.as_u64()).unwrap_or(0);
            let output_tokens = usage_obj.get("outputTokens").and_then(|v| v.as_u64()).unwrap_or(0);
            state.insert("usage".to_string(), serde_json::json!({
