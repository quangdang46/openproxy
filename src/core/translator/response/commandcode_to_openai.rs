...
pub fn commandcode_to_openai_response(chunk: &Value, state: &mut serde_json::Map<String, Value>) -> Vec<Value> {
    ...
    let event = if let Some(s) = chunk.as_str() {
        let line = s.trim();
        if line.is_empty() || line == "[DONE]" { return vec![]; }
        let json = line.strip_prefix("data:").map(|s| s.trim()).unwrap_or(line);
        if json.is_empty() { return vec![]; }
    ...
    if event_type.is_none() { return vec![]; }
    let event_type = event_type.unwrap();

    if !state.contains_key("responseId") {
        state.insert("responseId".to_string(), Value::String(format!("chatcmpl-{}", chrono::Utc::now().timestamp_millis())));
        state.insert("created".to_string(), Value::Number(chrono::Utc::now().timestamp().into()));
        state.insert("model".to_string(), event.get("model").cloned().unwrap_or(Value::String("commandcode".to_string())));
    ...
    let tool_index_by_id = state.get("toolIndexById").cloned().unwrap_or(serde_json::json!({}));
    ...
    match event_type {
        "text-delta" => {
            let text = event.get("text").or_else(|| event.get("delta")).and_then(|v| v.as_str()).unwrap_or("");
            ...
            state.insert("chunkIndex".to_string(), Value::Number((chunk_index + 1).into()));
        }
        "reasoning-delta" => {
        ...
        "tool-input-start" => {
            let id = event.get("id").or_else(|| event.get("toolCallId")).and_then(|v| v.as_str()).unwrap_or("");
            ...
            let idx = if let Some(existing) = tool_idx_by_id.get(id) {
                ...
                state.insert("toolIndex".to_string(), Value::Number((tool_index + 1).into()));
                tool_index
            };
        ...
        "tool-input-delta" => {
            let id = event.get("id").or_else(|| event.get("toolCallId")).and_then(|v| v.as_str());
            if id.is_none() { return vec![]; }
            let id = id.unwrap();
            let tool_idx_by_id = state.get("toolIndexById").cloned().unwrap_or(serde_json::json!({}));
        ...
        "tool-call" => {
            ...
            let id = id.unwrap();
            let tool_idx_by_id = state.get("toolIndexById").cloned().unwrap_or(serde_json::json!({}));
            if tool_idx_by_id.get(id).is_some() { return vec![]; }
        ...
        "finish-step" => {
            if let Some(reason) = event.get("finishReason").and_then(|v| v.as_str()) {
                state.insert("finishReason".to_string(), Value::String(map_finish_reason(reason).to_string()));
            }
        ...
        "finish" => {
            let finish_reason = event.get("finishReason").and_then(|v| v.as_str())
            ...
            if let Some(usage) = total_usage {
                ...
                let total = usage.get("totalTokens").and_then(|v| v.as_u64()).unwrap_or(input_tokens + output_tokens);
        ...
        "error" => {
            ...
            let err_str = err_val.map(|v| serde_json::to_string(v).unwrap_or_else(|_| "unknown".to_string())).unwrap_or_else(|| "unknown".to_string());
            out.push(make_chunk(serde_json::json!({"content": format!("\n\n[CommandCode error: {}]", err_str)}), None));
            out.push(make_chunk(serde_json::json!({}), Some("stop")));
            state.insert("finishReason".to_string(), Value::String("stop".to_string()));
        }
        _ => {}
    }
