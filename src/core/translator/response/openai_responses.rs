...
pub fn chat_to_responses_response(chunk: &Value, state: &mut serde_json::Map<String, Value>) -> Vec<Value> {
    if !chunk.get("choices").and_then(|v| v.as_array()).map_or(false, |a| !a.is_empty()) {
        return vec![];
    }
    ...
    if !state.contains_key("started") {
        ...
        let resp_id = chunk.get("id").and_then(|v| v.as_str()).map(|s| format!("resp_{}", s))
    ...
    if let Some(reasoning) = delta.get("reasoning_content").and_then(|v| v.as_str()) {
        if !reasoning.is_empty() {
            if state.get("reasoningId").is_none() || state["reasoningId"].is_null() {
                ...
                state.insert("reasoningId".to_string(), Value::String(reasoning_id.clone()));
                state.insert("reasoningIndex".to_string(), Value::Number(idx.into()));

                emit(&mut events, state, "response.output_item.added", serde_json::json!({
                    "type": "response.output_item.added",
                    "output_index": idx,
                    "item": {"id": reasoning_id, "type": "reasoning", "summary": []}
                }));

                emit(&mut events, state, "response.reasoning_summary_part.added", serde_json::json!({
            ...
            let reasoning_idx = state.get("reasoningIndex").and_then(|v| v.as_u64()).unwrap_or(0);
            state["reasoningBuf"] = Value::String(format!("{}{}", state["reasoningBuf"].as_str().unwrap_or(""), reasoning));

            emit(&mut events, state, "response.reasoning_summary_text.delta", serde_json::json!({
    ...
    if let Some(content) = delta.get("content").and_then(|v| v.as_str()) {
        if !content.is_empty() {
            let mut msg_item_added = state.get("msgItemAdded").cloned().unwrap_or(serde_json::json!({}));
            let mut msg_content_added = state.get("msgContentAdded").cloned().unwrap_or(serde_json::json!({}));
            let mut msg_text_buf = state.get("msgTextBuf").cloned().unwrap_or(serde_json::json!({}));
            ...
            if msg_item_added.get(&idx_str).is_none() {
                ...
                emit(&mut events, state, "response.content_part.added", serde_json::json!({
            ...
            let msg_id = state
                ...
                .unwrap_or("")
                .to_string();
            emit(&mut events, state, "response.output_text.delta", serde_json::json!({
            ...
            let existing = msg_text_buf.get(&idx_str).and_then(|v| v.as_str()).unwrap_or("").to_string();
    ...
    if let Some(tool_calls) = delta.get("tool_calls").and_then(|v| v.as_array()) {
        let mut func_call_ids = state.get("funcCallIds").cloned().unwrap_or(serde_json::json!({}));
        let mut func_args_buf = state.get("funcArgsBuf").cloned().unwrap_or(serde_json::json!({}));
        let mut func_names = state.get("funcNames").cloned().unwrap_or(serde_json::json!({}));

        for tc in tool_calls {
            let tc_idx = tc.get("index").and_then(|v| v.as_u64()).unwrap_or(0);
            let tc_idx_str = tc_idx.to_string();
            let call_id = tc.get("id").and_then(|v| v.as_str()).unwrap_or("");
            let func_name = tc.get("function").and_then(|f| f.get("name")).and_then(|v| v.as_str()).unwrap_or("");
            ...
            if let Some(args) = tc.get("function").and_then(|f| f.get("arguments")).and_then(|v| v.as_str()) {
                if !args.is_empty() {
                    let ref_call_id = func_call_ids.get(&tc_idx_str).and_then(|v| v.as_str()).unwrap_or(call_id);
                    if !ref_call_id.is_empty() {
                        emit(&mut events, state, "response.function_call_arguments.delta", serde_json::json!({
                    ...
                    let existing = func_args_buf.get(&tc_idx_str).and_then(|v| v.as_str()).unwrap_or("").to_string();
    ...
    if choice.get("finish_reason").is_some() {
        let mut msg_item_added = state.get("msgItemAdded").cloned().unwrap_or(serde_json::json!({}));
        let mut msg_text_buf = state.get("msgTextBuf").cloned().unwrap_or(serde_json::json!({}));
        let mut msg_item_done = state.get("msgItemDone").cloned().unwrap_or(serde_json::json!({}));
        let func_call_ids = state.get("funcCallIds").cloned().unwrap_or(serde_json::json!({}));
        let func_args_buf = state.get("funcArgsBuf").cloned().unwrap_or(serde_json::json!({}));
        let func_names = state.get("funcNames").cloned().unwrap_or(serde_json::json!({}));
        let mut func_item_done = state.get("funcItemDone").cloned().unwrap_or(serde_json::json!({}));

        for (k, _) in msg_item_added.as_object().unwrap_or(&serde_json::Map::new()) {
            if msg_item_done.get(k).is_none() {
                ...
                let msg_id = state
                    ...
                    .unwrap_or("")
                    .to_string();
                let full_text = msg_text_buf.get(k).and_then(|v| v.as_str()).unwrap_or("").to_string();

                emit(&mut events, state, "response.output_text.done", serde_json::json!({
                ...
                emit(&mut events, state, "response.content_part.done", serde_json::json!({
                ...
                emit(&mut events, state, "response.output_item.done", serde_json::json!({
            ...
            && state.get("reasoningDone").and_then(|v| v.as_bool()) == Some(false)
        {
            ...
            emit(&mut events, state, "response.reasoning_summary_text.done", serde_json::json!({
            ...
            emit(&mut events, state, "response.reasoning_summary_part.done", serde_json::json!({
        ...
        for (k, v) in func_call_ids.as_object().unwrap_or(&serde_json::Map::new()) {
            if func_item_done.get(k).is_none() {
                ...
                let args = func_args_buf.get(k).and_then(|v| v.as_str()).unwrap_or("{}");
                let name = func_names.get(k).and_then(|v| v.as_str()).unwrap_or("");

                emit(&mut events, state, "response.function_call_arguments.done", serde_json::json!({
        ...
        if state.get("completedSent").and_then(|v| v.as_bool()) != Some(true) {
            ...
            emit(&mut events, state, "response.completed", serde_json::json!({
...
pub fn responses_to_chat_response(chunk: &Value, state: &mut serde_json::Map<String, Value>) -> Vec<Value> {
    if chunk.is_null() {
        ...
        let finish_reason = if state.get("toolCallIndex").and_then(|v| v.as_u64()).unwrap_or(0) > 0
        ...
        state.insert("finishReason".to_string(), Value::String(finish_reason.to_string()));
    ...
    let event_type = chunk.get("type").or_else(|| chunk.get("event")).and_then(|v| v.as_str());
    let data = chunk.get("data").unwrap_or(chunk);

    if state.get("started").and_then(|v| v.as_bool()) != Some(true) {
        state.insert("started".to_string(), Value::Bool(true));
        state.insert("chatId".to_string(), Value::String(format!("chatcmpl-{}", chrono::Utc::now().timestamp_millis())));
        state.insert("created".to_string(), Value::Number(chrono::Utc::now().timestamp().into()));
    ...
    match event_type {
        ...
        Some("response.output_item.added") => {
            let item_type = data.get("item").and_then(|i| i.get("type")).and_then(|v| v.as_str());
            if item_type == Some("function_call") || item_type == Some("custom_tool_call") {
                ...
                state.insert("currentToolCallId".to_string(), Value::String(call_id.clone()));

                let tool_idx = state.get("toolCallIndex").and_then(|v| v.as_u64()).unwrap_or(0);
                vec![serde_json::json!({
                    "id": chat_id,
                    "object": "chat.completion.chunk",
                    "created": created,
        ...
        Some("response.output_item.done") => {
            ...
            if item_type == Some("function_call") || item_type == Some("custom_tool_call") {
                let tool_idx = state.get("toolCallIndex").and_then(|v| v.as_u64()).unwrap_or(0);
                state.insert("toolCallIndex".to_string(), Value::Number((tool_idx + 1).into()));
            }
            vec![]
        }
        Some("response.completed") => {
            if let Some(response) = data.get("response") {
                if let Some(usage) = response.get("usage") {
                    ...
                    let cache_read = usage.get("input_tokens_details").and_then(|d| d.get("cached_tokens")).and_then(|v| v.as_u64()).unwrap_or(0);
                    ...
                    if cache_read > 0 {
                        usage_obj["prompt_tokens_details"] = serde_json::json!({"cached_tokens": cache_read});
                    }
        ...
        Some("error") | Some("response.failed") => {
            ...
            let error = data.get("error").or_else(|| data.get("response").and_then(|r| r.get("error")));
