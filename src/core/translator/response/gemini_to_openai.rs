...
pub fn gemini_to_openai_response(chunk: &Value, state: &mut HashMap<String, Value>) -> Vec<Value> {
    ...
    if !state.contains_key("messageId") {
        let msg_id = response.get("responseId")
            .and_then(|v| v.as_str())
            .unwrap_or("unknown")
            .to_string();
        let model = response.get("modelVersion")
            .and_then(|v| v.as_str())
            .unwrap_or("gemini")
            .to_string();
    ...
    let msg_id = state.get("messageId").and_then(|v| v.as_str()).unwrap_or("unknown").to_string();
    let model = state.get("model").and_then(|v| v.as_str()).unwrap_or("gemini").to_string();
    let mut func_idx = state.get("functionIndex").and_then(|v| v.as_u64()).unwrap_or(0) as usize;

    if let Some(content) = candidate.get("content") {
        if let Some(parts) = content.get("parts").and_then(|v| v.as_array()) {
            for part in parts {
                let has_thought_sig = part.get("thoughtSignature").or_else(|| part.get("thought_signature")).is_some();
                let is_thought = part.get("thought").and_then(|v| v.as_bool()).unwrap_or(false);

                if has_thought_sig {
                    if let Some(text) = part.get("text").and_then(|v| v.as_str()) {
                        if !text.is_empty() {
                            let delta_key = if is_thought { "reasoning_content" } else { "content" };
                    ...
                    if let Some(func_call) = part.get("functionCall") {
                        ...
                        let tool_call_id = format!("{}-{}-{}", raw_name, chrono::Utc::now().timestamp_millis(), func_idx);
    ...
    if let Some(finish_reason) = candidate.get("finishReason").and_then(|v| v.as_str()) {
        ...
        if let Some(usage_meta) = response.get("usageMetadata").or_else(|| chunk.get("usageMetadata")) {
            if let Some(usage_obj) = usage_meta.as_object() {
                ...
                let thoughts_tokens = usage_obj.get("thoughtsTokenCount").and_then(|v| v.as_u64()).unwrap_or(0);
                let mut candidates_tokens = usage_obj.get("candidatesTokenCount").and_then(|v| v.as_u64()).unwrap_or(0);
                let total_tokens = usage_obj.get("totalTokenCount").and_then(|v| v.as_u64()).unwrap_or(0);

                if candidates_tokens == 0 && total_tokens > 0 {
                    candidates_tokens = total_tokens.saturating_sub(prompt_tokens).saturating_sub(thoughts_tokens);
                }
                ...
                if let Some(cached) = usage_obj.get("cachedContentTokenCount").and_then(|v| v.as_u64()) {
                    if cached > 0 {
                        usage["prompt_tokens_details"] = serde_json::json!({ "cached_tokens": cached });
                    }
                }
                if thoughts_tokens > 0 {
                    usage["completion_tokens_details"] = serde_json::json!({ "reasoning_tokens": thoughts_tokens });
                }
