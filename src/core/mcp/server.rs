...
fn tool_table() -> Vec<ToolHandler> {
    vec![
        // ── Provider tools ────────────────────────────────────────────────
        mcp_tool!("provider_list", "List all provider connections with their status", json!({}), |state, _args| {
            let snap = state.db.snapshot();
            Ok(json!(snap.provider_connections))
        }),
        mcp_tool!("provider_create", "Create a new provider connection", json!({
        ...
        }), vec!["provider", "name", "api_key"], |state, args| {
            let provider = args.get("provider").and_then(Value::as_str).ok_or("Missing 'provider'")?;
            let name = args.get("name").and_then(Value::as_str).ok_or("Missing 'name'")?;
            let api_key = args.get("api_key").and_then(Value::as_str).ok_or("Missing 'api_key'")?;
            ...
            if let Some(url) = base_url {
                conn.provider_specific_data.insert("baseUrl".to_string(), Value::String(url.to_string()));
            }
        ...
        mcp_tool!("provider_delete", "Delete a provider connection by ID or name", json!({
            "id": { "type": "string", "description": "Provider ID or name" }
        }), vec!["id"], |state, args| {
            let id = args.get("id").and_then(Value::as_str).ok_or("Missing 'id'")?;
            let id_str = id.to_string();
            db_update_sync(&state.db, move |db| {
                db.provider_connections.retain(|c| c.id != id_str && c.name.as_deref().map(|n| n != &id_str).unwrap_or(true));
            })?;
            Ok(json!({ "success": true }))
        }),
        mcp_tool!("provider_test", "Test a provider connection by querying its status", json!({
            "id": { "type": "string", "description": "Provider ID or name" }
        }), vec!["id"], |state, args| {
            let id = args.get("id").and_then(Value::as_str).ok_or("Missing 'id'")?;
            let snap = state.db.snapshot();
            let conn = snap.provider_connections.iter().find(|c| c.id == id || c.name.as_deref().map(|n| n == id).unwrap_or(false))
                .ok_or_else(|| format!("Provider '{id}' not found"))?;
            let url = conn.provider_specific_data.get("baseUrl")
        ...
        // ── API Key tools ─────────────────────────────────────────────────
        mcp_tool!("key_list", "List all API keys (secrets excluded)", json!({}), |state, _args| {
            let snap = state.db.snapshot();
            let keys: Vec<Value> = snap.api_keys.iter().map(|k| json!({
                "id": k.id,
                "name": k.name,
        ...
        mcp_tool!("key_create", "Create a new API key", json!({
        ...
        mcp_tool!("key_delete", "Delete an API key by ID", json!({
        ...
        // ── Combo tools ───────────────────────────────────────────────────
        mcp_tool!("combo_list", "List all model combos", json!({}), |state, _args| {
            let snap = state.db.snapshot();
            Ok(json!(snap.combos))
        }),
        mcp_tool!("combo_create", "Create a new model combo", json!({
        ...
        }), vec!["name", "models"], |state, args| {
            let name = args.get("name").and_then(Value::as_str).ok_or("Missing 'name'")?;
            let models = args.get("models").and_then(Value::as_array).ok_or("Missing 'models'")?;
            let model_ids: Vec<String> = models.iter().filter_map(|m| m.as_str().map(String::from)).collect();
        ...
        // ── Proxy Pool tools ──────────────────────────────────────────────
        mcp_tool!("pool_list", "List all proxy pools", json!({}), |state, _args| {
            let snap = state.db.snapshot();
            Ok(json!(snap.proxy_pools))
        }),
        mcp_tool!("pool_create", "Create a new proxy pool", json!({
        ...
        }), vec!["name", "proxy_url"], |state, args| {
            let name = args.get("name").and_then(Value::as_str).ok_or("Missing 'name'")?;
            let proxy_url = args.get("proxy_url").and_then(Value::as_str).ok_or("Missing 'proxy_url'")?;
        ...
        mcp_tool!("pool_delete", "Delete a proxy pool by ID", json!({
        ...
        // ── Node tools ────────────────────────────────────────────────────
        mcp_tool!("node_list", "List all provider nodes", json!({}), |state, _args| {
            let snap = state.db.snapshot();
            Ok(json!(snap.provider_nodes))
        }),
        // ── Model tools ───────────────────────────────────────────────────
        mcp_tool!("models_list", "List all model aliases", json!({}), |state, _args| {
            let snap = state.db.snapshot();
            let aliases: Vec<Value> = snap.model_aliases.iter().map(|(alias, target)| {
                let (target_model, kind) = match target {
                    ModelAliasTarget::Path(path) => (path.clone(), None),
                    ModelAliasTarget::Mapping(m) => (m.model.clone(), m.extra.get("kind").and_then(|v| v.as_str()).map(String::from)),
                };
                json!({
                    "alias": alias,
                    "target": target_model,
        ...
        // ── Health tools ──────────────────────────────────────────────────
        mcp_tool!("health", "Get server health status", json!({}), |_state, _args| {
            Ok(json!({ "status": "ok", "version": env!("CARGO_PKG_VERSION") }))
        }),
        // ── Settings tools ────────────────────────────────────────────────
        mcp_tool!("settings_get", "Get current server settings", json!({}), |state, _args| {
            let snap = state.db.snapshot();
            Ok(crate::server::api::safe_settings_payload(&snap.settings))
        }),
        // ── Usage tools ───────────────────────────────────────────────────
        mcp_tool!("usage_status", "Get current usage tracking status", json!({}), |_state, _args| {
...
fn handle_resources_read(state: &AppState, id: Value, params: &Value) -> Value {
    ...
    let text = match uri {
        ...
        "openproxy://keys" => {
            let snap = state.db.snapshot();
            let sanitised: Vec<Value> = snap.api_keys.iter().map(|k| json!({
                "id": k.id,
                "name": k.name,
                "is_active": k.is_active,
                "created_at": k.created_at,
