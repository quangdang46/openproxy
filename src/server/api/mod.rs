...
pub(crate) fn safe_settings_payload_with_db_path(
    ...
    if let Some(fields) = value.as_object_mut() {
        ...
        let oidc_configured = settings.is_oidc_configured()
            || (std::env::var("OIDC_ISSUER").ok().is_some_and(|v| !v.is_empty())
                && std::env::var("OIDC_CLIENT_ID").ok().is_some_and(|v| !v.is_empty())
                && std::env::var("OIDC_CLIENT_SECRET").ok().is_some_and(|v| !v.is_empty()));
        ...
        fields.insert("oidcLoginLabel".to_string(), Value::String(oidc_login_label));
