...
async fn cleanup_1p_legacy() -> AnyhowResult<()> {
    ...
    let managed_names: std::collections::HashSet<String> = local_stdio_plugins()
        .into_iter()
        .map(|p| p.name)
        .collect();
...
async fn load_cowork_status() -> AnyhowResult<Value> {
    ...
    let stdio_names: std::collections::HashSet<String> = local_stdio_plugins()
        .into_iter()
        .map(|p| p.name)
        .collect();
    ...
    let plugins: Vec<Value> = managed_mcp
        ...
        .map(|m| {
            ...
            let tool_names = defaults
                ...
                .unwrap_or_else(|| {
                    Value::Array(bare.into_iter().map(Value::String).collect())
                });
            json!({
                "name": name,
                "url": m.get("url"),
