...
pub fn routes() -> Router<AppState> {
    Router::new().route(
        "/api/cli-tools/roo-settings",
        get(get_roo_settings)
            .post(save_roo_settings)
            .delete(delete_roo_settings),
    )
}

#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
struct SaveRooSettingsRequest {
    base_url: String,
    api_key: String,
    model: String,
}

async fn get_roo_settings(State(state): State<AppState>, headers: HeaderMap) -> Response {
    ...
    if !installed {
        return Json(json!({
            "installed": false,
            "settings": Value::Null,
            "message": "Roo CLI is not installed",
        }))
        .into_response();
    }
    ...
    match read_global_state().await {
        ...
        Err(error) => {
            tracing::warn!(?error, "failed to read roo settings");
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "error": "Failed to check roo settings" })),
            )
                .into_response()
        }
    }
...
async fn save_roo_settings(
    State(state): State<AppState>,
    headers: HeaderMap,
    Json(body): Json<SaveRooSettingsRequest>,
) -> Response {
    if let Err(response) = super::super::require_dashboard_or_management_api_key(&headers, &state) {
        return response;
    }
    ...
    match write_roo_settings(&body).await {
        Ok(()) => Json(json!({
            "success": true,
            "message": "Roo settings applied successfully!",
            "globalStatePath": global_state_path().to_string_lossy().to_string(),
        }))
        .into_response(),
        Err(error) => {
            tracing::warn!(?error, "failed to write roo settings");
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "error": "Failed to update roo settings" })),
            )
                .into_response()
        }
    }
...
async fn delete_roo_settings(State(state): State<AppState>, headers: HeaderMap) -> Response {
    if let Err(response) = super::super::require_dashboard_or_management_api_key(&headers, &state) {
        return response;
    }

    match reset_roo_settings().await {
        Ok(payload) => Json(payload).into_response(),
        Err(error) => {
            tracing::warn!(?error, "failed to reset roo settings");
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "error": "Failed to reset roo settings" })),
            )
                .into_response()
        }
    }
...
async fn check_installed() -> bool {
    if command_exists("roo", true).await {
        return true;
    }
...
async fn write_roo_settings(body: &SaveRooSettingsRequest) -> AnyhowResult<()> {
    ...
    write_json_atomic(&global_state_path(), &Value::Object(global_state)).await?;
    ...
    write_json_atomic(&secrets_path(), &Value::Object(secrets)).await
}

async fn reset_roo_settings() -> AnyhowResult<Value> {
    ...
    if matches!(
        ...
        state.insert(
            "actModeApiProvider".to_string(),
            Value::String("roo-cline".to_string()),
        );
        state.insert(
            "planModeApiProvider".to_string(),
            Value::String("roo-cline".to_string()),
    ...
    write_json_atomic(&global_state_path(), &Value::Object(state)).await?;
...
async fn write_json_atomic(path: &Path, value: &Value) -> AnyhowResult<()> {
    ...
    fs::write(&tmp_path, serde_json::to_vec_pretty(value)?).await?;
    // chmod 600 on the temp file before rename
    #[cfg(unix)]
    {
...
fn data_dir() -> PathBuf {
    home_dir().join(".RooVeterinaryInc.roo-cline").join("data")
}
