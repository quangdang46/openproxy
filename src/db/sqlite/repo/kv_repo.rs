...
mod tests {
    ...
    fn roundtrip() {
        ...
        db.with_transaction(|tx| set(tx, "modelAliases", "gpt", &Value::String("openai/gpt-4o".into()))).unwrap();
    ...
    fn get_all_scope() {
        ...
        db.with_transaction(|tx| {
            set(tx, "pricing", "openai", &serde_json::json!({"gpt-4o": 0.01}))?;
            set(tx, "pricing", "anthropic", &serde_json::json!({"claude": 0.015}))?;
            Ok(())
        }).unwrap();
    ...
    fn upsert_replaces() {
        let db = SqliteDb::open_in_memory().unwrap();
        db.with_transaction(|tx| set(tx, "kv", "k1", &Value::String("old".into()))).unwrap();
        db.with_transaction(|tx| set(tx, "kv", "k1", &Value::String("new".into()))).unwrap();
